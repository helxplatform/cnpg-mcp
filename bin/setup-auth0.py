...
def save_output_files(
    ...
    # Helm values snippet
    helm_values = f"""# Auth0 Configuration for Helm Chart
# This references secrets created by create_k8s_secrets.py from auth0-config.json

auth0:
  issuer: "https://{domain}"
  audience: "{api_identifier}"

# Environment variables will be loaded from secrets:
# - mcp-auth0-config (AUTH0_ISSUER, AUTH0_AUDIENCE)
# - mcp-auth0-mgmt (client-id, client-secret, domain, connection-id)
...
Examples:
  ...
  python setup_auth0_for_mcp.py --token YOUR_TOKEN --recreate-client
    ...
    try:
        ...
        print("📋 Next Steps:")
        print()
        print("1. Create Kubernetes secrets:")
        print("   python create_k8s_secrets.py --namespace mcp")
        print()
        print("2. Deploy your MCP server:")
        print("   helm upgrade --install mcp-server ./mcp-server-chart -n mcp")
        print()
